...
class Main {
    ...
    public Long mySol(String input) {
		...
		ArrayList<TreeSet<Integer>> circuits = coordinates.stream()
			.map(i -> new TreeSet<>(Collections.singleton(i.idxorigin)))
			.collect(Collectors.toCollection(ArrayList::new));
		...
		for (Line l: distances) {
			...
			if (circuits.size() == 1) lastjunc = l;
		}

		return lastjunc.mulX();
    }
